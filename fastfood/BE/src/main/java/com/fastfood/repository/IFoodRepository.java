...
public interface IFoodRepository extends JpaRepository<Food, Long> {
    ...
    @Query(value = "select * from food f  where f.flag_food=true and f.category_id_category like concat ('%',:categoryId,'%') order by f.id_food desc", nativeQuery = true)
    List<Food> getListFoodByCategory(@Param("categoryId") String categoryId);

    @Query(value = "select * from food f join category c on c.id_category=f.category_id_category where (c.name like concat ('%',:name,'%') or f.name like concat ('%',:name,'%')) and f.flag_food=true order by f.id_food desc", nativeQuery = true)
    List<Food> getListFoodBySearch(@Param("name") String name);

    @Query(value = "select * from food f  where f.flag_food=true and f.category_id_category like concat ('%',:categoryId,'%') order by f.id_food desc",
            countQuery = "select * from food f  where f.flag_food=true and f.category_id_category like concat ('%',:categoryId,'%') order by f.id_food desc",nativeQuery = true)
